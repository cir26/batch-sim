...
# file with pickling
def dif_rxn(condition, max_T, T_rate, t, dt, P_rate=0, min_P=None, P_delay=0, term_delay=0):  # returns list of reaction interaction differential equation for each species
    T = condition[11]
    ...
    if min_P is None:
        ...
        P = condition[12]
        t_check = load_data('t_check')
        if t_check != np.round(t, 2):
            Reactions = rxn(condition, k, K, T, P, ideal=False, max_iter=100, pickle_data=False)
            print('t: ', t)
        else:
            Reactions = rxn(condition, k, K, T, P, ideal=False, max_iter=100)
            t_check += dt
        store_data('t_check', t_check)
...
def kinetic_const(T,
                  TC):  # returns list of forward rxn constant, and list of reaction constant equilibrium (K=k/k_prime)
    rxn_const = {
...
def rxn(conc, k, K, T=None, P=None, ideal=True, max_iter=None, pickle_data=True):
    k1 = k[0]
    k2 = k[1]
    k3 = k[2]
    k4 = k[3]
    ...
    if pickle_data is True:
        # load pickles
        t12_store = load_data('t12')
        t21_store = load_data('t21')
        xw_store = load_data('xw')
        xcap_store = load_data('xcap')
        yw_store = load_data('yw')
        ycap_store = load_data('ycap')
        L_store = load_data('L')
        V_store = load_data('V')
        # append new data
        t12_store.append(t12)
        t21_store.append(t21)
        xw_store.append(xw[0])
        print('len of xw: ', len(xw_store))
        xcap_store.append(xcap[0])
        yw_store.append(yw[0])
        ycap_store.append(ycap[0])
        L_store.append(L[0])
        V_store.append(V[0])
        # save pickles
        store_data('t12', t12_store)
        store_data('t21', t21_store)
        store_data('xw', xw_store)
        store_data('xcap', xcap_store)
        store_data('yw', yw_store)
        store_data('ycap', ycap_store)
        store_data('L', L_store)
        store_data('V', V_store)
